...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    // NB https://github.com/zeit/now/issues/3086
    if (req.url.startsWith('/http/') || req.url.startsWith('/https/'))
        req.url = req.url.replace(/(\/https?)\//i, "$1://");
    ...
    if (corsAnywhere.targetWhitelist.length && corsAnywhere.targetWhitelist.indexOf(location.hostname) === -1) {
      res.writeHead(403, 'Forbidden', cors_headers);
      res.end('The target "' + location.hostname + '" was not whitelisted by the operator of this proxy.');
      return;
    }
